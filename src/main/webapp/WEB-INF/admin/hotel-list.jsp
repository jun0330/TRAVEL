...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
			...
			<a class="btn btn-primary radius" onclick="nav_add('增加酒店','toAddHotel','4','','510')" href="javascript:;">
			<i class="Hui-iconfont">&#xe600;</i> 增加酒店</a></span></div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach  items ="${hotel}" var = "h">
				<tr class="text-c">
					...
					<td>${h.hotelpic}</td>
					<td>${h.hotelname}</td>
					<td class="text-l">${h.hotelprice}</td>
					<!-- <td class="text-c"></td>
					<td></td>
					<td class="td-status"><span class="label label-success radius">***</span></td> -->
					<td class="td-manage">
						<a href="javascript:void(0)" ><button class="upHotel" onclick="upHotel(${h.hotelid},'${h.hotelpic}','${h.hotelname}',${h.hotelprice})">修改
							<i class="Hui-iconfont">&#xe6df;</i></button></a>
						<button class="delHotel" type="button"  title="${h.hotelid}"><i class="Hui-iconfont" style="cursor:pointer">删除&#xe6e2;</i></button>
					</td>
				</tr>
			</c:forEach>
			</tbody>
	...
	$(function(){
		...
		var id = $(this).attr("title");
			 layer.confirm('确定删除吗', {btn: ['确定','取消']}, function(){
				 $.ajax({
					    url:"http://localhost:5656/mvn-lvyou/travel/delHotel/"+id,
					    type: "DELETE",
					    success: function (data) {
					    	if(data==true){
								location.href="${pageContext.request.contextPath}/travel/showHotel"
							}
					    }
					  }); 
			 }, function(){ 
	 ...
	 function upHotel(id,img,name,price) {
		location.href="${pageContext.request.contextPath}/travel/toUpHotel?hotelId="+id+"&hotelPic="+img+"&hotelName="+name+"&hotelPrice="+price;
	  	} 
